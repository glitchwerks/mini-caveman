...
/caveman-compress CLAUDE.md
        ↓
detect file type        (no tokens — local check)
        ↓
refuse if sensitive     (no tokens — denylist check before reading)
        ↓
node compress.js spawns headless claude --print session
  (main agent never reads file — isolated subprocess)
        ↓
validate output         (no tokens — local structural check)
  checks: headings, code blocks, URLs, file paths, bullets
        ↓
if errors: targeted fix via another headless session   (tokens — patches only)
...
Only two things use tokens: initial compression + targeted fix if validation fails. Everything else is local Node.js.

The compression runs in an **isolated headless session** — the main agent never reads the file contents. The headless child runs with `CAVEMAN_DEFAULT_MODE=off` so it won't apply caveman style to its own output.

## Install

Compress is built in with the `caveman` plugin. Install `caveman` once, then use `/caveman-compress`.

**Requires:** Node.js (ships with Claude Code — no additional install needed)
...
| `.env`, `credentials.*`, `id_rsa`, `*.pem` | Refused (sensitive) |
